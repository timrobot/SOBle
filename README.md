...
Included is a provided python package for host-side control and teleop of the SO101 platform over BLE, plus USB **leader** arm mapping. Under the `STL/` folder you will find several 3D models for printing as well as a list of sourced parts, in case you wish to build the platform yourself.
...
leader = SO101Leader("/dev/ttyACM0")  # sub in the port of your leader here
leader.load_config("config.json")  # see examples/angular_config.json
...
`load_config` accepts either format:

- **SOBle** — top-level `"leader"` / `"follower"` with **J1…J6** as `[min, max]` pairs (see `examples/angular_config.json`)
- **Standard SO101** — top-level `"so101_leader"` / `"so101_follower"`, each with a `"joints"` object (`min_limit` / `max_limit` per joint, keyed by name and sorted by motor `id`)
...
Print the included 3D parts, located in `STL/`. Electronics may be purchased from a vendor of your choice, however this parts list includes some recommended links.
...
| Mini LiPo balance charger/discharger | 1   | $32       | [Amazon](https://www.amazon.com/B6-Battery-Charger-Discharger-Connectors/dp/B0F2H3XR6S)                     |
...
| USB-C female to 2-pin terminal      | 1   | $8        | [Amazon](https://www.amazon.com/cablecc-Repair-Solderless-Connector-Terminal/dp/B0FY2HXYVF)               |
| Test hook to female jumper wires    | 1   | $9     | [Amazon](https://www.amazon.com/dp/B0BJ627S1X)                                                            |
...
Required electronics cost **$133**, primarily due to the battery and charger. Optional electronics add an additional **$80** (USB cable, Pi Zero 2 W, camera), for a grand total of **$213**.
...
leader = SO101Leader("COM4") # Windows
